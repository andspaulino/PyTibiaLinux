...
class WalkToCoordinateTask(VectorTask):
    ...
    def calculateWalkpoint(self, context: Context):
        self.tasks = []
        # Guarda defensiva Linux: sem posição mundial válida não existe ponto
        # inicial seguro para o pathfinding.
        if context['radar']['coordinate'] is None or self.coordinate is None:
            return
