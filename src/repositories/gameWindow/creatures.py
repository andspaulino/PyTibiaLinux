...
def getCreatureReachability(
    gameWindowCreatures,
    coordinate: Coordinate,
    nonWalkableCoordinates=None,
):
    if (
        coordinate is None
        or len(coordinate) != 3
        or coordinate[2] < 0
        or coordinate[2] >= len(walkableFloorsSqms)
    ):
        return []
    localWalkable = getGameWindowWalkableFloorsSqms(
        walkableFloorsSqms[coordinate[2]],
        coordinate,
    )
    if localWalkable.shape != (11, 15):
        return []

    # O jogador precisa continuar sendo uma raiz válida mesmo quando um estado
    # transitório ou uma leitura visual atrasada aponta para o próprio SQM.
    localWalkable[5, 7] = 1
    for blockedCoordinate in nonWalkableCoordinates or []:
        if (
            blockedCoordinate is None
            or not hasattr(blockedCoordinate, '__len__')
            or len(blockedCoordinate) != 3
            or blockedCoordinate[2] != coordinate[2]
        ):
            continue
        row = blockedCoordinate[1] - coordinate[1] + 5
        column = blockedCoordinate[0] - coordinate[0] + 7
        if 0 <= row < 11 and 0 <= column < 15 and (row, column) != (5, 7):
            localWalkable[row, column] = 0

    for creature in gameWindowCreatures:
        slot = creature.get('slot') if isinstance(creature, dict) else None
        if slot is None or len(slot) != 2:
            continue
        column, row = slot
        if 0 <= row < 11 and 0 <= column < 15:
            localWalkable[row, column] = 0

    pathfinder = tcod.path.AStar(localWalkable, 0)
    reachable = []
    for creatureIndex, creature in enumerate(gameWindowCreatures):
        slot = creature.get('slot') if isinstance(creature, dict) else None
        if slot is None or len(slot) != 2:
            continue
        targetColumn, targetRow = slot
        if not (0 <= targetRow < 11 and 0 <= targetColumn < 15):
            continue
        bestDistance = None
        for rowOffset in range(-1, 2):
            for columnOffset in range(-1, 2):
                if rowOffset == 0 and columnOffset == 0:
                    continue
                approachRow = targetRow + rowOffset
                approachColumn = targetColumn + columnOffset
                if not (
                    0 <= approachRow < 11
                    and 0 <= approachColumn < 15
                ):
                    continue
                if (approachRow, approachColumn) == (5, 7):
                    bestDistance = 0
                    continue
                if localWalkable[approachRow, approachColumn] == 0:
                    continue
                ...
                if len(path) == 0:
                    continue
                pathDistance = len(path)
                if bestDistance is None or pathDistance < bestDistance:
    ...
    return sorted(
        reachable,
        key=lambda item: (item['distance'], item['index']),
    )


def getClosestReachableCreature(
    gameWindowCreatures,
    coordinate: Coordinate,
    nonWalkableCoordinates=None,
):
    reachable = getCreatureReachability(
        gameWindowCreatures,
        coordinate,
        nonWalkableCoordinates=nonWalkableCoordinates,
    )
    return reachable[0]['creature'] if reachable else None
...
def getGameWindowWalkableFloorsSqms(walkableFloorsSqms: np.ndarray, coordinate: Coordinate) -> np.ndarray:
    ...
    # A fatia NumPy original era uma view; inserir criaturas como obstáculos
    # podia modificar permanentemente a matriz global de caminhabilidade.
    return walkableFloorsSqms[
        yOfPixelCoordinate - 5:yOfPixelCoordinate + 6,
        xOfPixelCoordinate - 7:xOfPixelCoordinate + 8,
    ].copy()
